...
class MusicCollectionPresenter extends Form
{
	...
	protected function configureView()
	{
		...
		$this->view->attachEventHandler( "GetHistorySong", function( $historyID )
		{
			...
			$vals = $connection->prepare( "SELECT * FROM tblMusicHistory WHERE UserID = :UserID" . ( isset( $date ) ? "AND MusicHistoryID != " . $historyID . " AND RequestedAt < '" . $date->format( "Y-m-d H:i:s" ) . "'" : "" ) . " ORDER BY RequestedAt DESC LIMIT 1" );
			$vals->execute( [
